...
def resample_using_importance_weights(samples:list, log_weights:list, normalize=True, wandb_name=None, verbose=True):
    '''
    ...
    _ = calculate_effective_sample_size(weights, verbose=verbose)
    indices = np.random.choice(nsamples, size=nsamples, replace=True, p=weights)
    resamples = [samples[index] for index in indices]
...
def calculate_log_evidence(log_weights:list):
    '''
    Calculate the (contribution to) the Bayesian model evidence based on the weights
    '''
    n = len(log_weights)
    log_evidence = tc.logsumexp(tc.tensor(log_weights), dim=0)-tc.log(tc.tensor(n))
    return log_evidence
